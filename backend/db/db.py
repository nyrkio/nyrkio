...
class DBStore(object):
    ...
    async def list_orgs(self):
        ...
        agg_query: List[Dict] = [
            ...
            { '$group': { '_id': "$oauth_accounts.organizations.organization.id",
                        'org_name': { '$addToSet': "$oauth_accounts.organizations.organization.login" },
                        'org_url': { '$addToSet': "$oauth_accounts.organizations.organization.url" } } },
            { '$group': { '_id': 1, 'all_orgs_id': { '$push': "$_id" }, 'all_orgs_name': { '$push': "$org_name" } } }
        ]
