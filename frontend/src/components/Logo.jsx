export const Logo = () => {
  return (
    <div className="nyrkio-logo nyrkio-logo-default">
      <img
        src="https://nyrkio.com/p/logo/full/Brown/NyrkioLogo_Final_Full_Brown-800px.png"
        alt="Nyrkiö (logo)"
        className="nyrkio-logo-img nyrkio-logo-img-default"
      />
    </div>
  );
};

export const SmallLogo = () => {
  return (
    <div className="nyrkio-logo nyrkio-logo-default">
      <img
        src="https://nyrkio.com/p/logo/full/Brown/NyrkioLogo_Final_Full_Brown-200px.png"
        alt="Nyrkiö (logo)"
        className="nyrkio-logo-img nyrkio-logo-img-small "
      />
    </div>
  );
};

export const LogoSlogan = () => {
  return (
    <div className="container-fluid text-center nyrkio-title nyrkio-logo nyrkio-logo-default">
      <img
        src="https://nyrkio.com/p/logo/full/Brown/NyrkioLogo_Final_Full_Brown-200px.png"
        alt="Nyrkiö (logo)"
        className="nyrkio-logo-img nyrkio-logo-img-small "
      />
      <h1>For Faster Software</h1>
    </div>
  );
};

export const LogoSloganNarrow = () => {
  return (
    <div className="container-fluid text-center nyrkio-title nyrkio-logo nyrkio-logo-default col-sm-12 col-md-12 col-xl-12">
      <img
        src="https://nyrkio.com/p/logo/full/Brown/NyrkioLogo_Final_Full_Brown-200px.png"
        alt="Nyrkiö (logo)"
        className="nyrkio-logo-img nyrkio-logo-img-small "
      />
      <h1>For Faster Software</h1>
    </div>
  );
};
